...
The result is as the simulations suggest: Hartberg with a 26% chance of winning, with the most likely scoreline being 1-3.
...
That is, with the hypothesized better passing performance we expect the overall reception attack kill rate to increase to `r rd1(new_rec_att_kill*100)`% (up from `r rd1(rates[[1]]$rec_att_kill*100)`%).
...
Giving an increase in the match win probability (up from `r rd1(sim_result$pwin*100)`% to `r rd1(new_sim_result$pwin*100)`%).
